...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <xsl:template name="maketitle">
   ...
   <xsl:call-template name="makestring">
     ...
     <xsl:with-param name="n" select="$sz" />
   </xsl:call-template>
   <xsl:text>&#10;</xsl:text>
   <xsl:text>&#10;</xsl:text>
 </xsl:template>
  ...
  <xsl:template match="db:section" mode="tr">
    ...
    <xsl:if test=".//db:footnote-x">
      ...
      <xsl:for-each select=".//db:footnote">
        <xsl:number value="position() + $footnotessofar" format="&#10;[1] " />
  ...
  <xsl:template match="db:para" mode="tr">
    <xsl:apply-templates select="*|text()" mode="tr" />
    <xsl:text>&#10;</xsl:text>
  ...
  <xsl:template match="/">
    ...
    <xsl:text>Document created using XSLT/XPath stylesheet and a DocBook V5 input.</xsl:text>
  </xsl:template>
